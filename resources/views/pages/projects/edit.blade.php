...
@section('content')
   <div class="min-h-screen max-md:px-6 px-20 flex flex-col items-center pt-32">
      <div class="w-full bg">
            <h1 class="text-2xl font-semibold">Edit Projects</h1>
            <hr class="my-6 border-gray-200 sm:mx-auto" />

            <div class="bg-white shadow-sm p-6 mt-10 border rounded-xl">
               <form action="/projects/edit/{{ $project->id }}" method="POST" enctype="multipart/form-data">
                  @csrf
                  <div class="mt-2">
                  ...
                  <div class="mt-6">
                     ...
                     <input
                        ...
                        placeholder="0"
                        name="total_member"
                        value="{{ old('total_member', $project->total_member) }}"
                        class="w-full bg-gray-100 px-4 py-2 rounded-md mt-1" 
                        required/>
                  </div>
                  <div class="mt-6" hidden>
                     ...
                     <input
                        id="researcher"
                        type="number"
                        value="{{ old('title', $project->id_researcher) }}"
                        name="id_researcher"
                        class="w-full bg-gray-100 px-4 py-2 rounded-md mt-1" 
                        required/>
                  </div>
                  <div class="mt-6" hidden>
                     ...
                     <input
                        ...
                        type="number"
                        name="published_year"
                        value="{{ old('title', $project->published_year) }}"
                        class="w-full bg-gray-100 px-4 py-2 rounded-md mt-1" 
                        required/>
                  </div>
                  <div class="mt-6">
                     ...
                     <select
                        ...
                        required
                        class="w-full bg-gray-100 px-4 py-2 rounded-md mt-1">
                           @if (old('status', $project->status) === 'Complated')
                              <option value="Open">Open</option>
                              <option value="Active">Active</option>
                              <option value="Complated" selected>Complated</option>
                           @elseif(old('status', $project->status) === 'Active')
                              <option value="Open">Open</option>
                  ...
                  <div class="mt-6 bg-gray-100 p-2 rounded-xl">
                     <div class="flex justify-center py-6 text-gray-600">
                        @if ($project?->image)
                           <img src="{{ asset('storage/'.$project->image) }}" class="w-40 rounded-xl mx-auto mb-2" alt="project picture " id="imageOldPreview">
                        @endif
                        <svg id="ic-img"
                           @if ($project?->image)
                           style="display:none;"
                     ...
                     <input class="w-full bg-gray-100 border border-gray-300 px-4 py-2 rounded-md mt-1" id="image" type="file" name="image" accept="image/*" onchange="showPreview(event);">
                  </div>
                  <div class="mt-6">
                     <label for="description" class="text-gray-500">Description</label>
                     <input id="description" type="hidden" name="description" value="{!! old('description', $project->description) !!}" required>
    ...
    <script>
      function showPreview(event){
         if(event.target.files.length > 0){
            ...
            const oldImg =document.getElementById('imageOldPreview')
