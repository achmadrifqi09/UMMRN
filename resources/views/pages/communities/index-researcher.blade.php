      ...
      <div class="flex justify-between items-center max-sm:flex-col max-sm:items-start">
         <div>
            <h1 class="text-2xl font-semibold">Comunities Of Practice</h1>
               ...
               <p class="text-gray-400">Manage communities of practice</p>
            </div>
            <a class="bg-lime-400 px-6 py-1 rounded-xl text-slate-900 h-max whitespace-nowrap max-sm:mt-2" href="/communities/create">Add Comunities</a>
            </div>
      <hr class="my-6 border-gray-200 sm:mx-auto" />
   </div>
   <div class="mt-6 w-full">
      @if($datas?->isEmpty)
         <p class="py-4 bg-red-200 text-center rounded-lg">You've Never Created a Community</p>
      @endif
         ...
         @foreach ($datas?->communities as $community)
            <div class="bg-white shadow-md rounded-xl">
               ...
               <div class="mt-20 p-4 pb-6 text-center">  
                     <a class="text-xl text-slate-900" href="/communities/detail/{{ $community->id }}">
                        {{ $community?->name }}
                     </a>
                  <p class="text-base mt-2 text-gray-500">
                     {!! $community?->description !!}
                  </p>
                     <a class="mt-4 left-4 px-3 rounded-full py-2 bg-lime-400 flex justify-center items-center" href="/communities/manage/{{ $community->id }}">
